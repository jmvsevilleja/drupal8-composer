...
/**
 * @file
 * Enables modules and site configuration for a drupal8_profile site installation.
 */
 ...
 */
function drupal8_profile_form_install_configure_form_alter(&$form, FormStateInterface $form_state) {
  $form['site_information']['site_name']['#default_value'] = 'Drupal 8 Site';
  $form['#submit'][] = 'drupal8_profile_form_install_configure_submit';
}

/**
 * Submission handler to sync the contact.form.feedback recipient.
 */
function drupal8_profile_form_install_configure_submit($form, FormStateInterface $form_state) {
/*  $site_mail = $form_state->getValue('site_mail');
  ContactForm::load('feedback')->setRecipients([$site_mail])->trustData()->save();
*/
  $password = $form_state->getValue('account')['pass'];
  drupal8_profile_set_users_passwords($password);
}

/**
 * Sets the password of admin to be the password for all users.
 */
function drupal8_profile_set_users_passwords($admin_password) {
 ...
 */
function drupal8_profile_toolbar() {
...
/*  $items['experimental-profile-warning'] = [
  ...
  if ($admin_context->isAdminRoute()) {
    ...
    $items['experimental-profile-warning']['tab'] = [
      ...
      '#attached' => [
        'library' => ['drupal8_profile/toolbar-warning'],
      ],
    ];
  }
  return $items;*/
